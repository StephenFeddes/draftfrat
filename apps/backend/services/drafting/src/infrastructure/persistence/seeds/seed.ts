...
import { playerTable, footballPlayerTable, footballTeamTable } from "../tables";
...
async function seedFootballTeamTable() {
    ...
    await new Promise<void>((resolve, reject) => {
        fs.createReadStream("./dist/infrastructure/persistence/seeds/football_teams.csv")
            .pipe(csv())
            .on("data", (row) => {
                rows.push({
...
async function seedPlayerTable() {
    ...
    const rows: any[] = [];
    await new Promise<void>((resolve, reject) => {
        fs.createReadStream("./dist/infrastructure/persistence/seeds/players.csv")
            .pipe(csv())
            .on("data", (row) => {
                rows.push({
                    id: parseInt(row.id, 10),
                    average_draft_position:
                        row.average_draft_position.length > 0 ? parseFloat(row.average_draft_position) : null,
                    first_name: row.first_name,
                    last_name: row.last_name,
                    injury_status: row.injury_status || null,
                    years_experience: row.years_experience.length > 0 ? parseInt(row.years_experience, 10) : null,
...
async function seedFootballPlayerTable() {
    const rowCount: number = (await db.select({ count: count() }).from(footballPlayerTable))[0].count;
    ...
    const rows: any[] = [];
    await new Promise<void>((resolve, reject) => {
        fs.createReadStream("./dist/infrastructure/persistence/seeds/football_players.csv")
            .pipe(csv())
            .on("data", (row) => {
                rows.push({
                    ...
                    projected_rushing_attempts:
                        row.projected_rushing_attempts.length > 0 ? parseFloat(row.projected_rushing_attempts) : null,
                    projected_rushing_yards:
                        row.projected_rushing_yards.length > 0 ? parseFloat(row.projected_rushing_yards) : null,
                    projected_rushing_touchdowns:
                        row.projected_rushing_touchdowns.length > 0
                            ? parseFloat(row.projected_rushing_touchdowns)
                            : null,
                    projected_targets: row.projected_targets.length > 0 ? parseFloat(row.projected_targets) : null,
                    projected_receiving_yards:
                        row.projected_receiving_yards.length > 0 ? parseFloat(row.projected_receiving_yards) : null,
                    ...
                    projected_passing_attempts:
                        row.projected_passing_attempts.length > 0 ? parseFloat(row.projected_passing_attempts) : null,
                    projected_passing_yards:
                        row.projected_passing_yards.length > 0 ? parseFloat(row.projected_passing_yards) : null,
