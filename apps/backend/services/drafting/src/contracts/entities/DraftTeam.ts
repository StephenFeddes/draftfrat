...
export abstract class DraftTeam {
    ...
    constructor(id: number, settings: DraftSettings, players: Player[] = []) {
     ...
     * @returns void
     */
    public assignPlayers(players: Player[], positionIndex: number = 0): void {
    ...
    public toJSON(): object {
        ...
        for (let i = 0; i < Object.keys(this.assignments).length; i++) {
            jsonAssignments[Object.keys(this.assignments)[i]] = this.assignments[
                Object.keys(this.assignments)[i]
            ].map((player) => (player ? player.toJSON() : null));
        }
