...
/**
 * Represents a fantasy sport team roster.
 * - A roster consists of a team number, owner, and assignments of players to positions.
 */
export class Roster {
    ...
    /**
     * Determines if a player can be added to the team without causing an invalid assignment.
     *
     * @param player - The player to check.
     * @returns {boolean} - True if the player can be added, false otherwise.
     */
    ...
    private assignPlayers(players: Player[], positionIndex: number = 0): void {
        ...
        const playersEligibleForPosition: (Player | null)[] = players.filter(
            (player: Player) => this.canPlayPosition(position, player) && !this.getAssignedPlayers().includes(player),
        );
        ...
        // Recursively assign players to the position and next position until a valid assignment is found.
    ...
    /**
     * Determines if a player can play a given position.
     *
     * @param position - The position to check.
     * @param player - The player to check.
     * @returns {boolean} - True if the player can play the position, false otherwise.
     * @example canPlayPosition("defense", { name: "Player", positions: ["defense", "bench"] }) => true;
     * @example canPlayPosition("kicker", { name: "Player", positions: ["defense", "bench"] }) => false;
     */
    private canPlayPosition(position: string, player: Player): boolean {
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            for (const combination of remainingCombinations) {
                ...
                const newPositionCombination: string = JSON.stringify(
                    newPlayerCombination.map((player: Player | null) => (player ? player.positions.sort() : ["None"])),
                );
    ...
    /**
     * Gets the assignments for the team.
     *
     * @returns An object containing the assignments for each position on the team.
     */
    public getAssignments(): Record<string, (Player | null)[]> {
