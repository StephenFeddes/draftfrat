...
export class FootballDraftTeam {
    private readonly id: number;

    private assignments: {
        [key in FootballPositionEnum]: (FootballPlayer | null)[];
    } = {
    ...
    private readonly spotsPerPosition: {
        [key in FootballPositionEnum]: number;
    };
    ...
    constructor(id: number, settings: FootballDraftSettings, players: FootballPlayer[] = []) {
        this.spotsPerPosition = {
     ...
     * @returns void
     */
    public addPlayer(player: FootballPlayer): void {
        // Get all players that have been assigned to the team.
        const players: FootballPlayer[] = this.getAssignedPlayers();
     ...
     * @returns void
     */
    public assignPlayers(players: FootballPlayer[], positionIndex: number = 0): void {
        ...
        const positions: FootballPositionEnum[] = Object.keys(
            this.spotsPerPosition,
        ) as FootballPositionEnum[];

        // Get the position to assign players to.
        const position: FootballPositionEnum = positions[positionIndex];
        ...
        const assignedPlayers: FootballPlayer[] = this.getAssignedPlayers();

        const playersEligibleForPosition: (FootballPlayer | null)[] = players.filter(
            (player: FootballPlayer) =>
                player.canPlayPosition(position) && !this.getAssignedPlayers().includes(player),
        );
        ...
        const playerCombinations: (FootballPlayer | null)[][] = this.generatePlayerCombinations(
     ...
     */
    private generatePlayerCombinations(
        players: (FootballPlayer | null)[],
        playersPerCombination: number,
    ): (FootballPlayer | null)[][] {
        ...
        const playerCombinations: (FootballPlayer | null)[][] = [];
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            for (const combination of remainingCombinations) {
                ...
                const newPositionCombination: string = JSON.stringify(
                    newPlayerCombination.map((player: FootballPlayer | null) =>
    ...
    public toJSON(): object {
        return {
            quarterback: this.assignments[FootballPositionEnum.QUARTERBACK].map((player) =>
                player ? player.toJSON() : null,
            ),
            runningBack: this.assignments[FootballPositionEnum.RUNNING_BACK].map((player) =>
                player ? player.toJSON() : null,
            ),
            wideReceiver: this.assignments[FootballPositionEnum.WIDE_RECEIVER].map((player) =>
                player ? player.toJSON() : null,
            ),
            tightEnd: this.assignments[FootballPositionEnum.TIGHT_END].map((player) =>
                player ? player.toJSON() : null,
            ),
            flex: this.assignments[FootballPositionEnum.FLEX].map((player) =>
                player ? player.toJSON() : null,
            ),
            bench: this.assignments[FootballPositionEnum.BENCH].map((player) =>
                player ? player.toJSON() : null,
            ),
            kicker: this.assignments[FootballPositionEnum.KICKER].map((player) =>
                player ? player.toJSON() : null,
            ),
            defense: this.assignments[FootballPositionEnum.DEFENSE].map((player) =>
                player ? player.toJSON() : null,
            ),
        };
    }
